...
<body>
    ...
    <p>Pada hari <b>{{ $hari_ini }}</b> tanggal <b>{{ $tanggal_ini }}</b>, bulan <b>{{ $bulan_ini }}</b>,
        tahun <b>{{ $tahun_ini }}</b>, bertempat di <b>{{ $industri }}</b></p>
    ...
    <table class="footer-table">
        <tr>
            ...
            <td style="width: 50%;">
                <p style="margin: 0;">Karawang, {{ $tanggal_surat }}<br>Pihak Sekolah,</p>
                @if (!empty($qr_signature))
                    <img src="{{ $qr_signature }}" alt="QR TTD" style="width: 90px; margin: 10px 0;">
                @else
                    <br><br><br><br>
                @endif
                <p style="margin: 0;"><b><u>{{ $wakasek_name }}</u></b><br>NIP.{{ $wakasek_nip }}</p>
            </td>
        </tr>
    </table>
</body>
